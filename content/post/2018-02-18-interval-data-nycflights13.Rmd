...
viz_nycflights13 <-
  flights_jfk %>% 
  group_by(wd, hh) %>% 
  summarise_at(vars(dep_delay), funs(mean(., na.rm = TRUE))) %>% 
  ungroup() %>% 
  mutate(hh = hh - 1) %>% 
  ...
  labs(
    ...
    caption = 
      paste0(
        "There appears to be more delays near the end of the day.\n",
...
```{r save_viz_nycflights13, include = FALSE, eval = params$eval}
path_save_banner <- "static/img/interval-data-nycflights13/viz_nycflights13-banner.png"
ggsave(viz_nycflights13, file = path_save_banner, units = "in", width = 8, height = 4)
```
