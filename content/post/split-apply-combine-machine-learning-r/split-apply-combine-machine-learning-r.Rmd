...
# NOTE: Set export logicals to FALSE for blog post.
config <-
  list(
    export_data = FALSE,
...
```{r fit_rpart, results = "hide", eval = params$eval}
fit_rpart <-
  train(
    form = fmla_diabetes,
...
```{r fit_rpart_unscaled, results = "hide", eval = params$eval}
fit_rpart_unscaled <-
  train(
    form = fmla_diabetes,
    data = PimaIndiansDiabetes,
...
```{r fit_rpart_unscaled_show, echo = FALSE}
fit_rpart_unscaled
```
...
```{r fit_rf, results = "hide", eval = params$eval}
fit_rf <-
  train(
    form = fmla_diabetes,
    data = PimaIndiansDiabetes,
...
```{r fits_diabetes_tree_1, results = "hide", eval = params$eval}
set.seed(42)
...
# NOTE: Choosing to export the entire environment instead of breaking things
...
```{r fits_diabetes_tree, results = "hide", eval = params$eval}
set.seed(42)
...
viz_summ_diabetes_bymethod <-
  ...
  labs(
    title = str_wrap("Cross Validation Accuracy of Split-Apply-Combine {caret} Models", 100),
