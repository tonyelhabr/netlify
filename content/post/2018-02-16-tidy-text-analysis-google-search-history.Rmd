...
path_save <-
  file.path("static", "data", "google_activity.RData")
```

```{r load, include = FALSE, eval = !params$eval}
# NOTE: Load only for blog post.
# NOTE: Delete `params` becuase it has been saved in RData file.
# rm("params")
load(path_save)
# params$eval <- FALSE
```
...
Next, I create a `config` list in order to emulate what one might 
do with a parameterized RMarkdown report (where the `config` would be a 
...
```{r data_io, echo = FALSE, eval = params$eval}
path_data <- "https://raw.githubusercontent.com/tonyelhabr/googleactivity/master/data/data-tony-cleaned.csv"
# path_data <- "data-ext/data-tony-cleaned.rds"
# saveRDS(data, file = path_data)
# data <- readRDS(path_data)
# readr::write_csv(data, path = gsub("\\.rds", ".csv", path_data)
...
```{r visualize_time_usage, echo = FALSE, results = "hide", fig.show = "hide", eval = params$eval}
lab_subtitle_all <-
  paste0(
    "From ",
...
viz_time_hh_2 <-
  ...
  scale_fill_manual(values = config$color_main) +
  geom_violin(size = 0) +
  geom_hline(
...
```{r unigrams, results = "hide", eval = params$eval}
...
```{r unigram_show, echo = FALSE}
unigrams %>% select(word) %>% count(word, sort = TRUE)
```

```{r bigrams, results = "hide", eval = params$eval}
# References:
# + https://www.tidytextmining.com/
# + https://www.tidytextmining.com/ngrams.html
bigrams <-
...
```{r viz_unigram_cnts, results = "hide", fig.show = "hide", eval = params$eval}
# Reference:
...
```{r viz_unigram_cnts_show, echo = FALSE}
viz_unigram_cnts
```

```{r viz_bigram_cnts, results = "hide", fig.show = "hide", eval = params$eval}
viz_bigram_cnts <-
  visualize_cnts(
    data = bigrams,
    color = config$color_main,
...
```{r viz_bigram_cnts_show, echo = FALSE}
viz_bigram_cnts
```
...
```{r viz_unigram_cnts_wordcloud, results = "hide", fig.show = "hide", eval = params$eval}
...
```{r viz_unigram_cnts_wordcloud_show, echo = FALSE}
viz_unigram_cnts_wordcloud
```

```{r viz_bigram_cnts_wordcloud, results = "hide", fig.show = "hide", eval = params$eval}
...
```{r viz_bigram_cnts_wordcloud_show, echo = FALSE}
viz_bigram_cnts_wordcloud
```
...
```{r unigram_freqs, results = "hide", eval = params$eval}
...
```{r unigram_freqs_show, echo = FALSE}
unigram_freqs
```

```{r bigram_freqs, results = "hide", eval = params$eval}
bigram_freqs <-
  compute_freqs(
    data = bigrams,
    colname_cnt = "word"
...
```{r bigram_freqs_show, echo = FALSE}
bigram_freqs
```
...
```{r compute_corrs_func, results = "hide", eval = params$eval}
# Reference:
# + https://www.tidytextmining.com/ngrams.html
...
```{r viz_corrs_network, results = "hide", fig.show = "hide", eval = params$eval}
# Reference:
# + http://varianceexplained.org/r/seven-fav-packages/.
unigram_corrs_list <-
  compute_corrs(
...
```{r viz_corrs_network_show, echo = FALSE}
viz_corrs_network
```
...
```{r unigram_bytime, results = "hide", eval = params$eval}
# Reference:
...
```{r unigram_bytime_show, echo = FALSE}
unigram_bytime
```
...
```{r unigram_bytime_models_slopes, results = "hide", eval = params$eval}
...
```{r unigram_bytime_models_slopes_show, echo = FALSE}
unigram_bytime_models_slopes
```
...
```{r viz_unigram_change_bytime, results = "hide", fig.show = "hide", eval = params$eval}
num_top_change <- 5
...
```{r viz_unigram_change_bytime_show, echo = FALSE}
viz_unigram_change_bytime
```
...
```{r viz_tfidf, results = "hide", fig.show = "hide", eval = params$eval}
# References:
...
```{r save, include = FALSE, eval = params$eval}
