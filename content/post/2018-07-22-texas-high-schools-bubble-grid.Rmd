...
```{r schools_raw_sf, eval = params$eval}
schools_raw_sf <-
  path_dest %>%
  sf::st_read() %>%
...
```{r path_schools_sf, eval = params$eval}
path_schools_sf <- 
  file.path(config$dir_data, "schools_sf.shp")
```

```{r schools_sf_export, eval = params$eval}
schools_sf %>% 
  st_write(path_schools_sf, delete_dsn = TRUE)
```

```{r schools_sf_import, eval = params$eval}
schools_sf <-
  st_read(path_schools_sf)
```

```{r counties_dl, eval = params$eval}
...
```{r counties_raw_sf, eval = params$eval}
counties_raw_sf <-
  path_dest_counties %>%
  sf::st_read() %>%
  mutate_if(is.factor, funs(as.character)) %>% 
...
```{r counties_sf, eval = params$eval}
counties_sf <-
  counties_raw_sf %>%
...
```{r path_counties_sf, eval = params$eval}
path_counties_sf <- 
  file.path(config$dir_data, "counties_sf.shp")
```

```{r counties_sf_export, eval = params$eval}
counties_sf %>% 
  st_write(path_counties_sf, delete_dsn = TRUE)
```

```{r counties_sf_import, eval = params$eval}
counties_sf <-
  st_read(path_counties_sf)
```

```{r cities_dl, eval = params$eval}
...
```{r cities_sf_raw, eval = params$eval}
cities_raw_sf <-
  path_dest_cities %>%
  sf::st_read() %>% 
  mutate_if(is.factor, funs(as.character)) %>% 
...
```{r path_cities_sf, eval = params$eval}
path_cities_sf <- 
  file.path(config$dir_data, "cities_sf.shp")
```

```{r cities_sf_export, eval = params$eval}
cities_sf %>% 
  st_write(path_cities_sf, delete_dsn = TRUE)
```

```{r cities_sf_import, eval = params$eval}
cities_sf <-
  st_read(path_cities_sf)
```


```{r hwys_dl, eval = params$eval}
...
```{r hwys_sf_raw, eval = params$eval}
hwys_raw_sf <-
  path_dest_hwys %>%
  sf::st_read() %>%
  mutate_if(is.factor, funs(as.character)) %>% 
...
```{r path_hwys_sf, eval = params$eval}
path_hwys_sf <- 
  file.path(config$dir_data, "hwys_sf.shp")
```

```{r hwys_sf_export, eval = params$eval}
hwys_sf %>% 
  st_write(path_hwys_sf, delete_dsn = TRUE)
```

```{r hwys_sf_import, eval = params$eval}
hwys_sf <-
  st_read(path_hwys_sf)
```

```{r schools_tea, eval = params$eval}
schools_tea <-
...
```{r schools_grid, include = TRUE, eval = params$eval}
counties_grid_sf <-
  counties_sf %>%
...
```{r path_schools_grid_sf, eval = params$eval}
path_schools_grid_sf <- 
  file.path(config$dir_data, "schools_grid_sf.shp")
```

```{r schools_grid_sf_export, eval = params$eval}
schools_grid_sf %>% 
  st_write(path_schools_grid_sf, delete_dsn = TRUE)
```

```{r schools_grid_sf_import, eval = params$eval}
schools_grid_sf <-
...
```{r viz_schools_grid_export, eval = params$eval}
...
```{r tx_counties, eval = params$eval}
tx_counties <- 
  teplot::get_map_data_county(state = "tx") %>% 
  as_tibble()
```

```{r tx_counties_debug, eval = params$eval}
...
```{r schools_tea_filt_join, eval = params$eval}
...
```{r viz_schools_chlrexport, eval = params$eval}
teproj::export_ext_png(
  viz_schools_chlr,
  export = config$export_viz,
  dir = config$dir_viz,
